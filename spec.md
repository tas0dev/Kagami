...
単なる描画エンジンではなく、**「システムとユーザーの保護すべき領域を厳格に守る」** ことを最大の設計思想としています。各アプリケーションの描画領域を完全に分離し、Kagamiのみがそれらを合成（Composite）して画面という「鏡」に映し出します。
...
一般のアプリケーションが `System Layer` や `Status Layer` のフラグを立ててウィンドウを作成しようとした場合、`kagamid` は要求元のプロセスID（PID）の権限をカーネルに問い合わせ、許可されていない場合は要求を破棄します。
